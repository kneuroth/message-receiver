...
module.exports.receiveMessage = async (req: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  ...
  // TODO: At this level, Telegram requires that if the message made it to the bot, it should get a 200. 
  // Otherwise it will keep trying to send the previous message
  if (!req.body) {
    return {
      statusCode: 400,
      body: 'No body'
  ...
  if (parseScore.valid) {
    ...
    try {
      ...
      const pk = String(update.message.from.id) +'_' + String(update.message.chat.id) +'_'+ format(fromUnixTime(update.message.date), 'yyyy-MM-dd')
