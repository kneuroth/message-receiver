import { ROCKET_SVG, FIRE_HEART_SVG, FIRE_SVG, CLAP_SVG, NEUTRAL_FACE_SVG, POOP_SVG, CLOWN_SVG, MAD_FACE_SVG, CRY_FACE_SVG, SANTA_SVG, COLD_GUY_SVG, PRESENT_SVG, CHRISTMAS_TREE_SVG, SNOW_CLOUD_SVG, ROCK_SVG, POPPER_SVG } from "./svgs";

export const DEFAULT_SVG_SCORE_MAP = {
  0: ROCKET_SVG,
  1: FIRE_HEART_SVG,
  2: FIRE_SVG,
  3: CLAP_SVG,
  4: NEUTRAL_FACE_SVG,
  5: MAD_FACE_SVG,
  6: CRY_FACE_SVG,
  7: POOP_SVG,
  8: CLOWN_SVG,
}

export const CHRISTMAS_SVG_SCORE_MAP = {
  0: ROCKET_SVG,
  1: FIRE_HEART_SVG,
  2: PRESENT_SVG,
  3: CHRISTMAS_TREE_SVG,
  4: SANTA_SVG,
  5: COLD_GUY_SVG,
  6: SNOW_CLOUD_SVG,
  7: ROCK_SVG,
  8: CLOWN_SVG,
}

export const MATRIX_SVG_SCORE_MAP = {
  0: ROCKET_SVG,
  1: FIRE_HEART_SVG,
  2: PRESENT_SVG,
  3: CHRISTMAS_TREE_SVG,
  4: SANTA_SVG,
  5: COLD_GUY_SVG,
  6: SNOW_CLOUD_SVG,
  7: ROCK_SVG,
  8: CLOWN_SVG,
}

export const DEFAULT_PODIUM_SVG_MAP = {
  1: POPPER_SVG,
  2: FIRE_SVG,
  3: CLAP_SVG,
}

export const CHRISTMAS_PODIUM_SVG_MAP = {
  1: POPPER_SVG,
  2: PRESENT_SVG,
  3: CLAP_SVG,
}